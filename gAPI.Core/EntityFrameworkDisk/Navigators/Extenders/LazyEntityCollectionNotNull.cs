...
public class LazyEntityCollectionNotNull<TForeign, TPrimary> : ICollection<TForeign>
{
    ...
    public LazyEntityCollectionNotNull(
    ...
    {
        var primaryKey = getPrimaryKey(primary);
        foreach (var item in dbSet.Where(a => getForeignKey(a).Equals(primaryKey)))
        {
            yield return item;
        }
    }
